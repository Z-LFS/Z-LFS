...
static int f2fs_prepare_super_block(void)
{
  ...
  c.segs_per_sec = c.segs_per_sec * GRID_CNT;
  c.segs_per_zone = c.segs_per_sec * c.secs_per_zone;
  MSG(0, "Info: Grid count %d\n", GRID_CNT);
  MSG(0, "Info: Segments per section modified to %d\n", c.segs_per_sec);
#endif
	...
	MSG(1, "Info: zone aligned start offset: %lu\n",
					zone_align_start_offset);
	...
	set_sb(segment_count, (c.total_segments / c.segs_per_zone *
						c.segs_per_zone));
#if 0//META_FOR_ZNS
	//for ZNS, use 2 zones for checkpoint
...
#if GRID_STRIPE
  total_meta_segments += 6 * blkz_size_segs;
#else
...
//	set_sb(segment_count_nat_log, 2 * blkz_size_segs * meta_stripe_cnt);
  ...
  //  c.blks_per_seg * blkz_size_segs) - 2;
	...
	MSG(1, "\tseg0 blkaddr : 0x%x", get_sb(segment0_blkaddr));
	MSG(1, "\tcp_seg_count : 0x%x\n", get_sb(segment_count_ckpt));
	MSG(1, "\tsit_blkaddr : 0x%x, segment_count_sit : 0x%x\n", 
			get_sb(sit_blkaddr), get_sb(segment_count_sit));
	MSG(1, "\tnat_blkaddr : 0x%x, segment_count_nat : 0x%x\n", 
			get_sb(nat_blkaddr), get_sb(segment_count_nat));
	MSG(1, "\tssa_blkaddr : 0x%x, segment_count_ssa : 0x%x\n", 
			get_sb(ssa_blkaddr), get_sb(segment_count_ssa));
	MSG(1, "\tsit_log_blkaddr : 0x%x, segment_count_sit_log : 0x%x\n", 
			get_sb(sit_log_blkaddr), get_sb(segment_count_sit_log));
	MSG(1, "\tnat_log_blkaddr : 0x%x, segment_count_nat_log : 0x%x\n", 
			get_sb(nat_log_blkaddr), get_sb(segment_count_nat_log));
	MSG(1, "\tssa_log_blkaddr : 0x%x, segment_count_ssa_log : 0x%x\n", 
			get_sb(ssa_log_blkaddr), get_sb(segment_count_ssa_log));
	MSG(1, "\tmain_blkaddr : 0x%x, segment_count_main : 0x%x\n", 
			get_sb(main_blkaddr), get_sb(segment_count_main));
	MSG(1, "\tsection count : 0x%x, segment count : 0x%x\n", 
			get_sb(section_count), get_sb(segment_count));
