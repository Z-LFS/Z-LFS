...
static int f2fs_prepare_super_block(void)
{
	...
	int i;
#if META_FOR_ZNS
	uint32_t blkz_cap_blks, blkz_size_segs;
	uint32_t zone_needed;
	// 获取zone size和zone capacity
  	uint32_t blkz_size_blks = c.devices[0].zone_blocks;
	blkz_cap_blks = c.devices[0].zone_cap_blocks[0];
	blkz_size_segs = blkz_size_blks / c.blks_per_seg;
	//META_STRIPING
	//CONFIGURE
	int meta_stripe_cnt = META_STRIPE_CNT; // no meta striping
	MSG(1, "Info: Blocks per zone: %u\n",
					blkz_size_blks);
	MSG(1, "Info: META_STRIPE_CNT: %u\n",
					META_STRIPE_CNT);
#endif
#if GRID_STRIPE
  uint32_t reserved_meta_segs = 0;
  c.segs_per_sec = c.segs_per_sec * GRID_CNT; // 乘以GRID_CNT构成一个superzone
  c.segs_per_zone = c.segs_per_sec * c.secs_per_zone;
  MSG(0, "Info: Grid count %d\n", GRID_CNT);
  MSG(0, "Info: Segments per section modified to %d [refers to how many segments a zone size contain.]\n", c.segs_per_sec);
#endif
...
#if META_FOR_ZNS
	zone_size_bytes = blkz_size_blks * blk_size_bytes;
#else
	zone_size_bytes =
	...
	MSG(1, "Info: zone aligned start offset: %lu, c.start_sector: %u\n."
		  "DEFAULT_SECTOR_SIZE: %u, DEFAULT_SECTORS_PER_BLOCK: %u, F2FS_BLKSIZE_BITS: %u,\n"
		 "zone_size_bytes: %u, c.zoned_mode: %d\n",
		zone_align_start_offset, c.start_sector, DEFAULT_SECTOR_SIZE,
		 DEFAULT_SECTORS_PER_BLOCK, F2FS_BLKSIZE_BITS, zone_size_bytes, c.zoned_mode);

	MSG(0, "Info: zone aligned segment0 blkaddr: %u\n",
					get_sb(segment0_blkaddr));
	MSG(0, "Info: block_count: %llu\n",
					get_sb(block_count));
	...
	for (i = 0; i < c.ndevs; i++) {
		if (i == 0) {
			c.devices[i].total_segments =
				...
				segment_size_bytes;
			MSG(0, "Info: c.devices[i].total_segments: %u, c.devices[i].total_sectors: %lu, c.sector_size: %u, zone_align_start_offset: %lu, segment_size_bytes: %u\n",
					c.devices[i].total_segments,c.devices[i].total_sectors,c.sector_size,zone_align_start_offset,segment_size_bytes);
		...
		// 但是total segments是所有device的总和
		c.total_segments += c.devices[i].total_segments;
	}
	//META_FOR_ZNS
	set_sb(segment_count, (c.total_segments / c.segs_per_zone *
						c.segs_per_zone));
	MSG(0, "INFO: segment_count: %u, c.total_segments: %u [1][by tt]\n", get_sb(segment_count), c.total_segments);
#if 0//META_FOR_ZNS
	//for ZNS, use 2 zones for checkpoint【TODO为什么设置了if 0】
	set_sb(segment_count_ckpt, 2 * c.segs_per_zone);
#else
...
#if META_FOR_ZNS
#if GRID_STRIPE
	if(get_sb(sit_blkaddr) % (c.blks_per_seg * blkz_size_segs)){
#else
	if(get_sb(sit_blkaddr) % (c.blks_per_seg * c.segs_per_zone)){
#endif //GRID_STRIPE
		MSG(1, "\tError: sit_blkaddr is not aligned "
				"with zone!!!\n");
		return -1;
	}
#endif //META_FOR_ZNS
	blocks_for_sit = SIZE_ALIGN(get_sb(segment_count), SIT_ENTRY_PER_BLOCK);
	MSG(1, "(%s : %d) original blocks_for_sit = %u\n",
			__func__, __LINE__, blocks_for_sit);

	//META_FOR_ZNS
#if META_FOR_ZNS
	/*DBG(1, "zone_cap : %u\n", blkz_cap_blks);
	if(blkz_cap_blks && blocks_for_sit > blkz_cap_blks) {
		blocks_for_sit = ADJUSTED_BLK_COUNT(blocks_for_sit, 
				blkz_cap_blks, blkz_size_blks);
	}
	printf("(%s : %d) modified blocks_for_sit = %u\n", 
			__func__, __LINE__, blocks_for_sit);
	
	sit_segments = SEG_ALIGN(blocks_for_sit);
		
//	zone_cap = zone_cap * DEFAULT_SECTORS_PER_BLOCK; //blocks
	//uint32_t sit_zones;
	//sit_zones = SIZE_ALIGN(sit_segments, c.segs_per_zone);
	//set_sb(segment_count_sit, 2 * sit_zones * c.segs_per_zone);
	set_sb(segment_count_sit, 2 * ZONE_ALIGNED_SEG_COUNT(sit_segments));
	*/
	zone_needed = ((blocks_for_sit - 1) / blkz_cap_blks) + 1;
	sit_segments = SEG_ALIGN(blocks_for_sit);
	// 设置为sit_segments的两倍==>备份？
	set_sb(segment_count_sit, sit_segments * 2);
	// 设置nat_blkaddr为SIT区域后的起始地址【实际分配的是两倍的zone needed，因为是在merge area中，且以zone size为单位(blkz_size_blks)】
	set_sb(nat_blkaddr, get_sb(sit_blkaddr) +
			2 * zone_needed * blkz_size_blks);
	MSG(1, "sit zone needed : %u\n", zone_needed); // 输出的zone need仍然是1，是没有乘以2的结果
#else
	sit_segments = SEG_ALIGN(blocks_for_sit);
	// 这里也是两倍=》备份
	set_sb(segment_count_sit, sit_segments * 2);
...
#if META_FOR_ZNS
#if GRID_STRIPE
	if(get_sb(nat_blkaddr) % (c.blks_per_seg * blkz_size_segs)){
#else
	if(get_sb(nat_blkaddr) % (c.blks_per_seg * c.segs_per_zone)){
#endif
		MSG(1, "\tError: nat_blkaddr is not aligned "
				"with zone!!!\n");
		MSG(1, "(%s:%d)nat_blkaddr\n", __func__, __LINE__);
		return -1;
	}
#endif
	...
	blocks_for_nat = SIZE_ALIGN(total_valid_blks_available,
			NAT_ENTRY_PER_BLOCK);
	MSG(1, "(%s : %d) original blocks_for_nat = %u\n",
			__func__, __LINE__, blocks_for_nat);
#if 0 // META_FOR_ZNS
	if(blkz_cap_blks) {
		blocks_for_nat = ADJUSTED_BLK_COUNT(blocks_for_nat, 
				blkz_cap_blks, blkz_size_blks);
	}
	printf("(%s : %d) modified blocks_for_nat = %u\n",
			__func__, __LINE__, blocks_for_nat);
	...
	//META_FOR_ZNS - do not consider this (option -i)
	if (c.large_nat_bitmap) {
		...
		MSG(1, "(%s : %d) original max nat segs = %u\n",
			__func__, __LINE__, max_nat_segments);
		MSG(1, "(%s : %d) cp payload = %u\n",
			__func__, __LINE__, get_sb(cp_payload));
#if 0
		set_sb(segment_count_nat, 
				ZONE_ALIGNED_SEG_COUNT(get_sb(segment_count_nat)));
#endif
		set_sb(segment_count_nat, get_sb(segment_count_nat) * 2);
	}
...
#if META_FOR_ZNS
#if GRID_STRIPE
	if(get_sb(ssa_blkaddr) % (c.blks_per_seg * blkz_size_segs)){
#else
	if(get_sb(ssa_blkaddr) % (c.blks_per_seg * c.segs_per_zone)){
#endif
		MSG(1, "\tError: ssa_blkaddr is not aligned "
				"with zone!!!\n");
		MSG(1, "(%s:%d)ssa_blkaddr\n", __func__, __LINE__);
		return -1;
	}
	blocks_for_nat = get_sb(segment_count_nat) / 2 * c.blks_per_seg;
	zone_needed = ((blocks_for_nat - 1) / blkz_cap_blks) + 1;
	set_sb(ssa_blkaddr, get_sb(nat_blkaddr) + 
			2 * zone_needed * blkz_size_blks);
	MSG(1, "nat zone needed : %u\n", zone_needed);
#else 
	...
	/*
	if(blkz_cap_blks) {
		blocks_for_ssa = ADJUSTED_BLK_COUNT(blocks_for_ssa,
				blkz_cap_blks, blkz_size_blks);
	}
	set_sb(segment_count_ssa, 
			2 * ZONE_ALIGNED_SEG_COUNT(SEG_ALIGN(blocks_for_ssa)));
	*/
	set_sb(cp_payload, get_sb(cp_payload) + 1);
...
#else
//  total_meta_segments += 6 * c.segs_per_zone;
  total_meta_segments += 6 * c.segs_per_zone * meta_stripe_cnt;
#endif
// adjust SSA segment count to align to blkzone
	diff = total_meta_segments % blkz_size_segs;
	if (diff) {
		set_sb(segment_count_ssa, get_sb(segment_count_ssa) +
			(blkz_size_segs - diff)); // 多余的segments分给ssa
  }
  MSG(1, "Debug: total meta segs : %lu, diff : %lu, seg count ssa : %u\n",
    total_meta_segments, diff, get_sb(segment_count_ssa));
#else // META_FOR_ZNS
	set_sb(segment_count_ssa, SEG_ALIGN(blocks_for_ssa));
...
#if META_FOR_ZNS
	blocks_for_ssa = get_sb(segment_count_ssa) * c.blks_per_seg;
	zone_needed = ((blocks_for_ssa - 1) / blkz_cap_blks) + 1;
	set_sb(segment_count_ssa, get_sb(segment_count_ssa) * 2);
	MSG(1, "ssa zone needed : %u\n", zone_needed);
  MSG(1, "\tDebug: modify SSA seg count to: %u\n", get_sb(segment_count_ssa));

  //log area
#if GRID_STRIPE
	set_sb(segment_count_sit_log, 2 * blkz_size_segs);
//	set_sb(segment_count_sit_log, 2 * blkz_size_segs * meta_stripe_cnt);
#else
	set_sb(segment_count_sit_log, 2 * c.segs_per_zone * meta_stripe_cnt);
#endif // GRID_STRIPE
	//set_sb(sit_log_blkaddr, get_sb(ssa_blkaddr) + get_sb(segment_count_ssa) *
	//		c.blks_per_seg);
	set_sb(sit_log_blkaddr, get_sb(ssa_blkaddr) + 
			2 * zone_needed * blkz_size_blks);
#if GRID_STRIPE
	set_sb(segment_count_nat_log, 2 * blkz_size_segs);
//	set_sb(segment_count_nat_log, 2 * blkz_size_segs * meta_stripe_cnt);
#else
	set_sb(segment_count_nat_log, 2 * c.segs_per_zone * meta_stripe_cnt);
#endif // GRID_STRIPE
	set_sb(nat_log_blkaddr, get_sb(sit_log_blkaddr) + 
			get_sb(segment_count_sit_log) * c.blks_per_seg);

#if GRID_STRIPE
	set_sb(segment_count_ssa_log, 2 * blkz_size_segs * meta_stripe_cnt);
#else
	set_sb(segment_count_ssa_log, 2 * c.segs_per_zone * meta_stripe_cnt);
#endif // GRID_STRIPE
	set_sb(ssa_log_blkaddr, get_sb(nat_log_blkaddr) + 
			get_sb(segment_count_nat_log) * c.blks_per_seg);

  // main area
	set_sb(main_blkaddr, get_sb(ssa_log_blkaddr) +
			get_sb(segment_count_ssa_log) * c.blks_per_seg);
#if GRID_STRIPE 
  //diff = get_sb(main_blkaddr) % (c.blks_per_seg * c.segs_per_zone); 
  diff = (get_sb(main_blkaddr) - get_sb(segment0_blkaddr)) %
    (c.blks_per_seg * c.segs_per_zone); 
  if(diff) {
    MSG(1, "Debug: main blkaddr is not aligned by section: diff %lu blocks\n", diff);
    diff = (c.blks_per_seg * c.segs_per_zone) - diff;
    set_sb(main_blkaddr, get_sb(main_blkaddr) + diff);
  }
#endif // GRID_STRIPE
	if(get_sb(main_blkaddr) % (blkz_size_segs)){
		MSG(1, "\tError: main_blkaddr is not aligned "
				"with zone!!!\n");
		MSG(1, "(%s:%d) main blkaddr: %u\n", __func__, __LINE__, get_sb(main_blkaddr));
		return -1;
	}
	// superblock is not included in total meta segs
	total_meta_segments = (get_sb(main_blkaddr) - get_sb(segment0_blkaddr)) /
    c.blks_per_seg;
	total_meta_zones = ZONE_ALIGN(total_meta_segments *	c.blks_per_seg);
	//total_meta_zones = SIZE_ALIGN(total_meta_segments *	c.blks_per_seg, 
  //  c.blks_per_seg * blkz_size_segs) - 2;
	MSG(1, "(%s:%d) total meta segs : %lu, total meta blkzones : %lu\n",
			__func__, __LINE__, total_meta_segments, total_meta_zones);
#else // META_FOR_ZNS
	...
	if (c.zoned_mode) {
		...
		if (c.devices[0].zoned_model == F2FS_ZONED_HM &&
				c.devices[0].nr_zones < main_blkzone){
			MSG(0, "\tError: Device does not have enough "
					"write zones for F2FS volume (at least %lu needed)\n",
					main_blkzone);
			MSG(1, "(%s:%d) device space\n", __func__, __LINE__);
			return -1;
		}
#else
		if (c.devices[0].zoned_model == F2FS_ZONED_HM &&
	...
	set_sb(segment_count_main, get_sb(section_count) * c.segs_per_sec);
  	...
  	MSG(1, "(%s:%d) segment_count_main : %u [by tt]\n",
			__func__, __LINE__, get_sb(segment_count_main));
	/*
	...
	MSG(1, "(%s:%d) c.overprovision: %f, c.reserved_segments: %u, f2fs_get_usable_segments(sb): %u, section_count:%u. by tt\n",
			__func__, __LINE__, c.overprovision, c.reserved_segments, f2fs_get_usable_segments(sb), get_sb(section_count));
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		c.overprovision = 0;
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		...
		// 获取main area中的6个log stream的起始segment number
		c.cur_seg[CURSEG_HOT_NODE] = 0;
...
#if SEP_SSA
	printf("(%s:%d) c.cur_seg[CURSEG_HOT/WARM/COLD_NODE/DATA] segno: %u %u %u %u %u %u\n"
			, __func__, __LINE__, 
        c.cur_seg[CURSEG_HOT_NODE],
...
#if META_FOR_ZNS
	MSG(1, "(META_FOR_ZNS) Info: \n");
	MSG(1, "\tseg0 blkaddr : 0x%x = %u", get_sb(segment0_blkaddr), get_sb(segment0_blkaddr));
	MSG(1, "\tcp_seg_count : 0x%x = %u\n", get_sb(segment_count_ckpt), get_sb(segment_count_ckpt));
	MSG(1, "\tsit_blkaddr : 0x%x = %u, segment_count_sit : 0x%x = %u\n", 
			get_sb(sit_blkaddr), get_sb(sit_blkaddr), get_sb(segment_count_sit), get_sb(segment_count_sit));
	MSG(1, "\tnat_blkaddr : 0x%x = %u, segment_count_nat : 0x%x = %u\n", 
			get_sb(nat_blkaddr), get_sb(nat_blkaddr), get_sb(segment_count_nat), get_sb(segment_count_nat));
	MSG(1, "\tssa_blkaddr : 0x%x = %u, segment_count_ssa : 0x%x = %u\n", 
			get_sb(ssa_blkaddr), get_sb(ssa_blkaddr), get_sb(segment_count_ssa), get_sb(segment_count_ssa));
	MSG(1, "\tsit_log_blkaddr : 0x%x = %u, segment_count_sit_log : 0x%x = %u\n", 
			get_sb(sit_log_blkaddr), get_sb(sit_log_blkaddr), get_sb(segment_count_sit_log), get_sb(segment_count_sit_log));
	MSG(1, "\tnat_log_blkaddr : 0x%x = %u, segment_count_nat_log : 0x%x = %u\n", 
			get_sb(nat_log_blkaddr), get_sb(nat_log_blkaddr), get_sb(segment_count_nat_log), get_sb(segment_count_nat_log));
	MSG(1, "\tssa_log_blkaddr : 0x%x = %u, segment_count_ssa_log : 0x%x = %u\n", 
			get_sb(ssa_log_blkaddr), get_sb(ssa_log_blkaddr), get_sb(segment_count_ssa_log), get_sb(segment_count_ssa_log));
	MSG(1, "\tmain_blkaddr : 0x%x = %u, segment_count_main : 0x%x = %u\n", 
			get_sb(main_blkaddr), get_sb(main_blkaddr), get_sb(segment_count_main), get_sb(segment_count_main));
	MSG(1, "\tsection count : 0x%x = %u, segment count : 0x%x = %u\n", 
			get_sb(section_count), get_sb(section_count), get_sb(segment_count), get_sb(segment_count));
...
static int f2fs_init_sit_area(void)
{
	...
	uint32_t zone_cnt, zone_size, unusable_blks;
	uint32_t zone_cap, zone_cap_segs;
  ...
  /* size in bytes*/
	zone_size = c.devices[0].zone_blocks * blk_size; 
	zone_cap = c.devices[0].zone_cap_blocks[0] * blk_size;
	unusable_blks = (zone_size - zone_cap) % seg_size;
	zone_cap_segs = zone_cap / seg_size;
	
	zone_cnt = (get_sb(segment_count_sit)/2) * c.blks_per_seg;
	zone_cnt = (zone_cnt - 1) / c.devices[0].zone_cap_blocks[0] + 1;
	zone_cnt *= 2;
...
//	get_sb(log_blocksize);
	MSG(1, "(%s %d) sit_blkaddr : %u, blk_size : %u, zone_cnt : %d\n", __func__, __LINE__,
			get_sb(sit_blkaddr), blk_size, zone_cnt);

	DBG(1, "\tFilling sit area at byte offset 0x%08"PRIx64"\n", sit_seg_addr);
	for (j = 0; j < zone_cnt; j+=2){
		sit_seg_addr = get_sb(sit_blkaddr) * (__u64) blk_size;
		sit_seg_addr +=  j * zone_size;
		// j = 0: sit_seg_addr = sit_seg_addr
		// j = 2: sit_seg_addr = sit_seg_addr + 2 * zone_size【TODO】交错写入不相邻的两个zone
		MSG(1, "(%s %d) sit_seg_byte_adddr %lu\n", __func__, __LINE__,
			sit_seg_addr);
		//sit_seg_addr = get_sb(sit_blkaddr) << get_sb(log_ blocksize) + j * zone_size;
		for (index = 0; index < zone_cap_segs; index++) {
		...
		/* fill usable blocks of last segment in blks*/
		if(unusable_blks){
			if (dev_fill(zero_buf, sit_seg_addr, seg_size - unusable_blks)) {
				MSG(1, "\tError: While zeroing out the sit area "
						"on disk!!!, unsuable_blks(%u)\n", unusable_blks);
				free(zero_buf);
				return -1;
			}
		}
...
#endif
#if META_FOR_ZNS 
//init nat after write root nat
static int __f2fs_init_nat_area(void){
	uint32_t blk_size, seg_size, zone_size;
	uint32_t zone_cnt, unusable_blks;
	uint32_t zone_cap, zone_cap_segs;
	uint32_t index = 0;
	...
	blk_size = 1 << get_sb(log_blocksize);
	seg_size = (1 << get_sb(log_blocks_per_seg)) * blk_size;
	//zone_size = get_sb(segs_per_sec) * seg_size; 
	zone_size = c.devices[0].zone_blocks * blk_size; 
	zone_cap = c.devices[0].zone_cap_blocks[0] * blk_size;
	unusable_blks = (zone_size - zone_cap) % seg_size;
	zone_cap_segs = zone_cap / seg_size;

	zone_cnt = (get_sb(segment_count_nat)/2) * c.blks_per_seg;
	zone_cnt = (zone_cnt - 1) / c.devices[0].zone_cap_blocks[0] + 1;
	zone_cnt *= 2;
	...
	//the first page was already written to device
	// 从nat_seg_addr + blk_size开始，是因为前面f2fs_update_nat_root已经写入了一个有效的nat block
	if (dev_fill(nat_buf, nat_seg_addr + blk_size, seg_size - blk_size)) {
	...
	// reset后面的nat area
	// index从1开始，是因为第0个segment的前面已经Reset了
	for (index = 1; index < zone_cap_segs; index++) {
	...
	MSG(1, "\t(__f2fs_init_nat_area) zeroing out the nat area offset 0x%lu, j = 0\n", nat_seg_addr);

	/* fill usable blocks of last segment in blks*/
	if(unusable_blks){
		if (dev_fill(nat_buf, nat_seg_addr, seg_size - unusable_blks)) {
			MSG(1, "\tError: While zeroing out the nat area "
					"on disk!!!\n");
			free(nat_buf);
			return -1;
	...
	/* fill another zone in NAT set 1 */
	// 【TODO】nat area: 0x300000和0x500000
	for (j = 2; j < zone_cnt; j+=2) {
		nat_seg_addr = get_sb(nat_blkaddr) * blk_size + j*zone_size;
		MSG(1, "\t(__f2fs_init_nat_area) zeroing out the nat area offset 0x%lu, j = 2\n", nat_seg_addr);
		for (index = 0; index < zone_cap_segs; index++) {
			if (dev_fill(nat_buf, nat_seg_addr, seg_size)) {
				MSG(1, "\tError: While zeroing out the nat area "
						"on disk!!!\n");
...
static int f2fs_write_check_point_pack(void)
{
	...
	//用block表示
	// << 1: 因为 checkpoint pack 中需要放两份（对应两个 checkpoint pack 或主/备拷贝）
	// 额外头部信息：CRC 校验值（8 bytes）
	// + F2FS_BLKSIZE - 1 → 典型的 “向上对齐到 block 边界
	nat_bits_blocks = F2FS_BYTES_TO_BLK((nat_bits_bytes << 1) + 8 +
						F2FS_BLKSIZE - 1);
	// 分配nat_bits_blocks个block的空间
	...
	set_cp(cur_node_segno[0], c.cur_seg[CURSEG_HOT_NODE]); // 0
	set_cp(cur_node_segno[1], c.cur_seg[CURSEG_WARM_NODE]); // 1022
	...
	// MAX_ACTIVE_NODE_LOGS = 8，因此从3开始，后面5个都设置为无效值0xffffffff
	...
	// lpf: lost +(plus) found
	// cur_node_blkoff[0]: HOT NODE
	// cur_data_blkoff[0]: HOT DATA
	// c.quota_inum = quota 文件的 inode block 数
	// c.lpf_inum = lost+found 目录的 inode block 数
	// _dnum是数据块数
	...
	// c.reserved_segments是在f2fs_prepare_superblock时根据OP设置
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO)) {
		...
		//【TODO】为什么这里是减6（因为需要用到6个log stream，这6个正在使用，因此不包含在free）
		set_cp(free_segment_count, f2fs_get_usable_segments(sb) - 6);
		set_cp(user_block_count, ((get_cp(free_segment_count) + 6 -
		get_cp(overprov_segment_count)) * c.blks_per_seg));
	}
	MSG(0, "【Info】: user_block_count: %llu, free_segment_count: %u, overprov_segment_count: %u, c.blks_per_seg: %u\n",
				get_cp(user_block_count), get_cp(free_segment_count), get_cp(overprov_segment_count), c.blks_per_seg);
	...
	set_cp(sit_ver_bitmap_bytesize, ((get_sb(segment_count_sit) / 2) <<
			get_sb(log_blocks_per_seg)) / 8); // 转换为byte需要除8

	set_cp(nat_ver_bitmap_bytesize, ((get_sb(segment_count_nat) / 2) <<
			 get_sb(log_blocks_per_seg)) / 8);
#if META_FOR_ZNS
	printf("(%s : %d) original sit_ver_bitmap_byte_size = %u\n"
			, __func__, __LINE__, get_cp(sit_ver_bitmap_bytesize));
	printf("(%s : %d) original nat_ver_bitmap_byte_size = %u\n"
			, __func__, __LINE__, get_cp(nat_ver_bitmap_bytesize));
	
	zone_count_ssa = (get_sb(segment_count_ssa) /2 ) / get_sb(segs_per_sec);
	set_cp(ssa_ver_bitmap_bytesize, zone_count_ssa / 8 + 1);
	DBG(1, "(%s : %d) cp payload size : %u\nssa_ver_bitmap_bytesize : %u(%u/8+1)\n", 
			__func__, __LINE__, get_sb(cp_payload),
			get_cp(ssa_ver_bitmap_bytesize),
			zone_count_ssa);
#endif
	...
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
	...
	cp_seg_blk = get_sb(segment0_blkaddr); // cp从segment0开始写入
	...
	// 如果有cp_payload，需要增加cp blkaddr
	for (i = 0; i < get_sb(cp_payload); i++) {
		cp_seg_blk++;
#if META_FOR_ZNS
		if (dev_write_block(cp_payload, cp_seg_blk)) {
#else
		// dev_fill_block是填充0
	...
	// 将下面5个数据compact到一个block中(sum_compact)
	...
	*/
	...
	// 1.nat journal
	journal = &sum->journal; // journal指向sum中的journal
	journal->n_nats = cpu_to_le16(1 + c.quota_inum + c.lpf_inum);
	// 记录root inode的nat信息到nat journal中
	...
	for (qtype = 0, i = 1; qtype < F2FS_MAX_QUOTAS; qtype++) {
		...
		journal->nat_j.entries[i].ne.ino = sb->qf_ino[qtype]; // nid=ino -> struct inode
		journal->nat_j.entries[i].ne.block_addr = cpu_to_le32(
				get_sb(main_blkaddr) +
				get_cp(cur_node_segno[0]) *
				c.blks_per_seg + i); // 【TODO】这个时候cur_node_segno[0]并没有增加
		i++;
	}
	...
	// 将nat journal compact到sum_compact_p中
	memcpy(sum_compact_p, &journal->n_nats, SUM_JOURNAL_SIZE);
	sum_compact_p += SUM_JOURNAL_SIZE; // 向后移动journal大小（因为已经写入了一个nat journal）
	...
	if (c.feature & cpu_to_le32(F2FS_FEATURE_RO))
		...
		journal->n_sits = cpu_to_le16(6); // root的sit

	journal->sit_j.entries[0].segno = cp->cur_node_segno[0]; // 设置sit的segno
	journal->sit_j.entries[0].se.vblocks =
				cpu_to_le16((CURSEG_HOT_NODE << 10) |
						(1 + c.quota_inum + c.lpf_inum)); // vblocks指定该segment包含多少个valid block。左移 10 位，说明高位部分记录段的 类型（这里是 hot node segment，热节点段）
	// 将0号bit设置为1，表示第0个block有效.第0个block是应该是root inode
	f2fs_set_bit(0, (char *)journal->sit_j.entries[0].se.valid_map);
	// 将1~quota_inum的bit都设置为1
	for (i = 1; i <= c.quota_inum; i++)
		f2fs_set_bit(i, (char *)journal->sit_j.entries[0].se.valid_map);
	// 如果有lost+found目录，也将对应的bit设置为1
	if (c.lpf_inum)
		f2fs_set_bit(i, (char *)journal->sit_j.entries[0].se.valid_map);
	...
	if (flags & CP_NAT_BITS_FLAG) {
		...
		*(__le64 *)nat_bits = get_cp_crc(cp);
		...
		//fill empty area to zero in zone and progress write pointer 
#if META_FOR_ZNS
		for(i = wp; i < cp_seg_blk; i++){
			DBG(2, "\tfill dummy page in zone, at offset 0x%08"PRIx64"\n",
					(uint64_t) i);
			if (dev_write_block(zero_buf, i)) {
				MSG(1, "\tError: fill empty area in zone!!!\n");
				goto free_cp_payload;
			}
		}
#endif
	...
	/* 写checkpoint pack 2（冗余副本）*/
	/* cp page 1 of check point pack 2
	 * Initialize other checkpoint pack with version zero
	 */
	// (1)写page1（包含 checkpoint header、版本号、有效数据等）
	cp->checkpoint_ver = 0; // checkpoint_ver设为0，表示这是备用checkpoint pack

	crc = f2fs_checkpoint_chksum(cp); // 重新计算并写入 checkpoint 校验和。
	*((__le32 *)((unsigned char *)cp + get_cp(checksum_offset))) =
							cpu_to_le32(crc);
	cp_seg_blk = get_sb(segment0_blkaddr) + c.blks_per_seg; // cp pack 2写到下一个segment中
...
static int f2fs_write_super_block(void)
{
  ...
  int ret;
	...
	// +F2FS_SUPER_OFFSET: 前1024字节保留给主引导记录（MBR） 或引导程序
  ...
  // 将rep信息传递给内核，内核会将对应的zone信息写入rep中
  ret = ioctl(dev->fd, BLKREPORTZONE, rep);
  if (ret != 0 || !rep->nr_zones) {
    ERR_MSG("ioctl BLKREPORTZONES failed\n");
  }
  
  if (ret) {
    MSG(1, "\tError: while reporting superblock zone\n");
  }
...
//  range.sector = sector;
//  nr_sectors = c.devices[0].zone_blocks;// * c.sectors_per_blk;
//  range.nr_sectors = nr_sectors;
  ret = ioctl(c.devices[0].fd, BLKFINISHZONE, &range);
  if (ret){
    MSG(1, "\tError: while finish superblock zone\n");
  } 
#endif
...
static int f2fs_write_root_inode(void)
{
	...
	raw_node->footer.nid = sb->root_ino; // 3
	raw_node->footer.ino = sb->root_ino; // 3
...
#if SEP_SSA
// normal data blk addr calculation
// 指向的是数据块地址hot data
  data_blk_nor += ((CURSEG_HOT_NODE - CURSEG_HOT_DATA)
     * c.segs_per_zone * c.blks_per_seg); // CURSEG_HOT_NODE: 3, CURSEG_HOT_DATA: 0
#else
	data_blk_nor +=	c.cur_seg[CURSEG_HOT_DATA] * c.blks_per_seg;
#endif
	raw_node->i.i_addr[get_extra_isize(raw_node)] = cpu_to_le32(data_blk_nor);

	// 初始化root inode的extent cache
	raw_node->i.i_ext.fofs = 0;
	raw_node->i.i_ext.blk_addr = 0;
	raw_node->i.i_ext.len = 0;

	// 但root node本身要写入的区域是hot node
	main_area_node_seg_blk_offset = get_sb(main_blkaddr);
	main_area_node_seg_blk_offset += c.cur_seg[CURSEG_HOT_NODE] *
					c.blks_per_seg; // c.cur_seg[CURSEG_HOT_NODE]初始化为0

	DBG(1, "\tWriting root inode (hot node) at 0x%x, c.cur_seg[CURSEG_HOT_NODE]: %x, c.blks_per_seg: 0x%x, at offset 0x%08"PRIu64"\n",
			get_sb(main_blkaddr),
			c.cur_seg[CURSEG_HOT_NODE],
...
static int f2fs_update_nat_root(void)
{
	...
	nat_blk->entries[get_sb(root_ino)].ino = sb->root_ino; // 3

	/* update node nat */
	nat_blk->entries[get_sb(node_ino)].block_addr = cpu_to_le32(1); //【TODO】node块地址待定
	nat_blk->entries[get_sb(node_ino)].ino = sb->node_ino; // 1

	/* update meta nat */
	nat_blk->entries[get_sb(meta_ino)].block_addr = cpu_to_le32(1); //【TODO】meta块地址待定
	nat_blk->entries[get_sb(meta_ino)].ino = sb->meta_ino; // 2

	nat_seg_blk_offset = get_sb(nat_blkaddr);

	DBG(1, "\tWriting nat root (nat area), at offset 0x%08"PRIx64"\n",
					nat_seg_blk_offset);
