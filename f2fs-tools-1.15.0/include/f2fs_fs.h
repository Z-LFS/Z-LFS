...
#if META_FOR_ZNS
	__le32 ssa_ver_bitmap_bytesize;
#endif
...
#if META_FOR_ZNS
static_assert(sizeof(struct f2fs_checkpoint) == 200, "");
#else
...
#if META_FOR_ZNS
extern int f2fs_print_zones(int, uint32_t); // for debugging
#endif
...
#define ZONE_ALIGNED_SEG_COUNT(seg_count) \
	SIZE_ALIGN(seg_count, c.segs_per_zone) * c.segs_per_zone
#define ADJUSTED_BLK_COUNT(blks, cap, size) \
	cap == size? blks : (((blks - 1) / cap) * size)
