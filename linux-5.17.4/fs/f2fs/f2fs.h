...
#define nat_in_log(lblock, i)		((lblock)->entries[i].ne)
#define nid_in_log(lblock, i)		((lblock)->entries[i].nid)
#endif
...
#if HOTNESS
    atomic_t i_access_count; /* file access count */
#endif
...
struct f2fs_sm_info {
	...
	char *ssa_bitmap; 			/* one bit per one zone */
...
#if META_FOR_ZNS
	else if (flag == SSA_BITMAP)
		return le32_to_cpu(ckpt->ssa_ver_bitmap_bytesize);
#endif
...
#if META_FOR_ZNS
		start_addr = start_addr + sbi->blocks_per_blkz;
#else
		start_addr += sbi->blocks_per_seg;
#endif
	return start_addr;
...
#if META_FOR_ZNS
int f2fs_sync_single_meta_page(struct page *page);
struct page *get_next_log_page(struct f2fs_sb_info *sbi, 
		int log_type);
int move_metadata_page(struct f2fs_sb_info *sbi, block_t offset, int type);
int advance_meta_zone_wp(struct f2fs_sb_info *sbi, block_t zoff,
		int cur_wp, int add, int type);
...
#if META_FOR_ZNS
// return zone cap in the unit of block number
// ref : segment.c :: 5376, f2fs_usable_zone_segs_in_sec
// WARN: consider every zone has same size and cap
static inline unsigned int meta_blks_zone_cap(struct f2fs_sb_info *sbi){
	...
	if (!FDEV(dev_idx).zone_capacity_blocks)
		return sbi->blocks_per_blkz;

	return FDEV(dev_idx).zone_capacity_blocks[zone_idx];
}
/*
 * blk_offset : offset in metadata
 * return zone number
 */
static inline int meta_boff_to_zoff(struct f2fs_sb_info *sbi, u32 blk_offset){
	int zone_offset;
	zone_offset = blk_offset / meta_blks_zone_cap(sbi);
	return zone_offset;
}
static inline int meta_zoff_to_boff(struct f2fs_sb_info *sbi, u32 zoff){
	int boff;
	boff = zoff * meta_blks_zone_cap(sbi); 
	return boff;
}
/*
 * blk_offset : offset in metadata
 * return block offset in a zone
 */
static inline int meta_boff_in_zone(struct f2fs_sb_info *sbi, u32 blk_offset){
	int boff_in_zone;
	boff_in_zone = blk_offset % meta_blks_zone_cap(sbi);
	return boff_in_zone;
}
static inline bool has_curlog_space(struct f2fs_sb_info *sbi, 
		int entries, int type){
	...
	if(type == SIT_LOG){
		used_blocks = SM_I(sbi)->sit_blks_in_log;
		blocks_needed = (entries / SIT_LOG_ENTRIES) + 1;
	} else if(type == NAT_LOG){
		used_blocks = NM_I(sbi)->nat_blks_in_log;
		blocks_needed = (entries / NAT_LOG_ENTRIES) + 1;
...
static inline block_t get_cur_meta_blkaddr(struct f2fs_sb_info *sbi, 
		block_t offset, block_t base_addr, char *bitmap, int ssa){
...
static inline block_t get_next_meta_blkaddr(struct f2fs_sb_info *sbi, 
		block_t offset, block_t base_addr, char *bitmap, int ssa){
	block_t blkaddr;
...
static inline block_t blocks_needed(struct f2fs_sb_info *sbi,
		u32 segment_cnt)
{
	u32 zone_cap = meta_blks_zone_cap(sbi);
	u32 zones_needed;
	...
	//blocks_needed = (zones_needed * sbi->segs_per_sec) << sbi->log_blocks_per_seg;
