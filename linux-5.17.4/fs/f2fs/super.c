    ...
    for (j = 0; j < 6; j++) {
      ...
      if (decisions[j] > 0) {
        ...
        if (curseg->wanted_size - change < min_wanted_size) {
          change = curseg->wanted_size - min_wanted_size;
        }
        curseg->wanted_size -= change;
        opened -= change;
...
#if META_FOR_ZNS
	if (sit_log_blkaddr + (segment_count_sit_log << log_blocks_per_seg) !=
							nat_log_blkaddr) {
		f2fs_info(sbi, "Wrong SIT log area boundary, start(%u) end(%u) blocks(%u)",
			  sit_log_blkaddr, nat_log_blkaddr,
			  segment_count_sit_log << log_blocks_per_seg);
		bound_err = 1;
	}
...
static int f2fs_scan_devices(struct f2fs_sb_info *sbi)
{
	...
	for (i = 0; i < max_devices; i++) {
		...
		if (max_devices == 1) {
			...
			pr_info("[f2fs_scan_devices]: Multi-device mount. device%d: %s, total_segments:%u, start_blk=%u, end_blk=%u ---ttt\n", i, FDEV(i).path, FDEV(i).total_segments, FDEV(i).start_blk, FDEV(i).end_blk);
...
#if META_FOR_ZNS
static int f2fs_check_meta_boundary(struct f2fs_sb_info *sbi)
{
	struct f2fs_super_block *raw_super = sbi->raw_super;
	u32 cp_blkaddr = le32_to_cpu(raw_super->cp_blkaddr);
	u32 sit_blkaddr = le32_to_cpu(raw_super->sit_blkaddr);
	...
	if (cp_blkaddr + blocks_needed(sbi, segment_count_ckpt) !=
							sit_blkaddr) {
		f2fs_info(sbi, "Wrong CP boundary, start(%u) end(%u) blocks(%u) blocks needed(%u)",
			  cp_blkaddr, sit_blkaddr,
			  segment_count_ckpt << log_blocks_per_seg,
	...
	if (nat_blkaddr + blocks_needed(sbi, segment_count_nat) !=
							ssa_blkaddr) {
		f2fs_info(sbi, "Wrong NAT boundary, start(%u) end(%u) blocks(%u), blocks_needed(%u)",
			  nat_blkaddr, ssa_blkaddr,
			  segment_count_nat << log_blocks_per_seg,
	...
	if (ssa_blkaddr + blocks_needed(sbi, segment_count_ssa) !=
							sit_log_blkaddr) {
		f2fs_info(sbi, "Wrong SSA boundary, start(%u) end(%u) blocks(%u), blocks needed(%u)",
...
try_onemore:
...
#if META_FOR_ZNS
	err = f2fs_check_meta_boundary(sbi);
	if (err) {
		f2fs_err(sbi, "Metadata area boundary error");
		goto free_devices;
	}
#endif
	...
	if (!f2fs_readonly(sb) && !test_opt(sbi, DISABLE_CHECKPOINT) &&
		...
		if (err) {
			f2fs_err(sbi,
			    "Failed to start F2FS merge_thread (%d)",
			    err);
...
static int __init init_f2fs_fs(void)
{
	...
	printk("module init");
	//printk("Log size is changed!! see f2fs.h");
