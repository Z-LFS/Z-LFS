...
static bool __is_merged_meta(struct page *page){
  ...
  if (page->index < SIT_I(sbi)->sit_base_addr ||
...
alloc_new:
	...
	tmp = bio_add_page(io->bio, bio_page, PAGE_SIZE, 0);
	if (tmp < PAGE_SIZE) {
		__submit_merged_bio(io);
		goto alloc_new;
	}
 ...
 */
//extern struct timespec64 ts_f2fs_mdp[6][2];
//extern unsigned long long mdp_time[6];
//extern unsigned long long mdp_cnt[6];
