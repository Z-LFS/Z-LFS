...
#include "zoned.h"
#include <linux/kernel.h> // dump stack
#include <linux/writeback.h> // write sum log
		...
		enum dirty_type dirty_type)
{
	...
	/* need not be added */
#if STRIPE 
	if (IS_CURSEG(sbi, segno) || is_inuse_seg(sbi, segno))
		return;
#else 
...
#if STRIPE
	if (segno == NULL_SEGNO || IS_CURSEG(sbi, segno) || is_inuse_seg(sbi, segno))
		return;
#else
	if (segno == NULL_SEGNO || IS_CURSEG(sbi, segno))
		return;
#endif
	//printk("(%s : %d) locate dirty seg: %u, %u", 
	//		__func__, __LINE__, segno, is_inuse_seg(sbi,segno));
...
/* For ZNS SSD, discard unit is section  */
#if !SEP_SSA
...
//		printk("(%s : %d) prefree segment to free: %u", 
//			__func__, __LINE__, segno);
...
//		printk("(%s : %d) tree insert", __func__, __LINE__);
	...
	memcpy(&head->footer, &sum_blk->footer, SUM_FOOTER_SIZE);
	
	// for versioning log
	ckpt_ver = cur_cp_version(F2FS_CKPT(sbi));
	head->cp_ver = ckpt_ver;
...
static inline void sum_blk_to_sum_log(struct f2fs_summary_block *sum_blk,
	...
	memcpy(raw_sum_log->entries, sum_blk->entries,
			SUM_ENTRY_SIZE);
	memcpy(&raw_sum_log->footer, &sum_blk->footer, 
			SUM_FOOTER_SIZE);
}
static int write_sum_log_page(struct f2fs_sb_info *sbi,
			unsigned int segno,
			struct f2fs_summary_block *sum_blk)
{
	struct page *page;
	struct f2fs_sum_log_block *raw_sum_log;
	int ret = 0;
	if(segno >= SM_I(sbi)->main_segments) {
//		printk("(%s : %d) error : invalid segno", __func__, __LINE__);
		f2fs_bug_on(sbi, 1);
		return -1;
	}

	page = get_next_log_page(sbi, SSA_LOG);
	if(!page){
		printk("(%s : %d) error : failed to get next log page", __func__, __LINE__);
		return -1;
	}
  //printk("write sum log page: segno %u, secno %u", segno, GET_SEC_FROM_SEG(sbi, segno));
	raw_sum_log = page_address(page);
	raw_sum_log->segno = cpu_to_le32(segno);
	raw_sum_log->cp_ver = cpu_to_le64(cur_cp_version(F2FS_CKPT(sbi)));

	sum_blk_to_sum_log(sum_blk, raw_sum_log);
	...
	if ((ret = f2fs_sync_single_meta_page(page))) {
		unlock_page(page);
...
static void reset_curseg(struct f2fs_sb_info *sbi, int type, int modified)
{
	...
	// printk(KERN_INFO "reset_curseg: type=%d, segno=%u, zone=%u, get_zone=%u\n",
    //    type, curseg->segno, curseg->zone, GET_ZONE_FROM_SEG(sbi, curseg->segno));
			...
			int type)
{
  ...
  unsigned int sum_blkaddr;

#if IGZO
  int target_IG = -1;
  struct f2fs_sm_info *sm_i = SM_I(sbi);
  unsigned short *alloc_IG_list;

  if (seg_type <= CURSEG_COLD_DATA) {
    alloc_IG_list = sm_i->data_alloc_IG;
  } else if (seg_type <=  CURSEG_COLD_NODE) {
    alloc_IG_list = sm_i->node_alloc_IG;
  } else {
    f2fs_bug_on(sbi, 1);
  }
#endif
  ...
  if ((curseg->active_end - curseg->active_start) % max_size < curseg->wanted_size) {
    curseg->active_end = (curseg->active_end + 1) % max_size;
    if (!curseg->active_end)
      curseg->active_end = max_size;
  }

  //current section is exhausted
  if (((segno + 1) % sbi->segs_per_sec >= 
    f2fs_usable_zone_segs_in_sec(sbi, segno))) { // for zone cap < zone size
	  get_sec_entry(sbi, segno)->inuse = 0;

//    printk("%s:%d a section is full : %u", __func__, __LINE__,
//      f2fs_usable_zone_segs_in_sec(sbi, segno));
    curseg->active_zones[curseg->cursor] = NULL_SEGNO;
#if IGZO
    alloc_IG_list[GET_IG_FROM_SEG(sbi, segno)] = CURSEG_NOT_ALLOC;
#endif
  }
  
  // adjust cursor
  if (curseg->cursor >= curseg->active_end) {
    spin_lock(&curseg->inactive_lock);

  // stripe size decrease by monitor thread
    curseg->inactive_zones[curseg->inactive_end] = 
      curseg->active_zones[curseg->cursor];
    curseg->inactive_end++;
    curseg->active_zones[curseg->cursor] = NULL_SEGNO;

    spin_unlock(&curseg->inactive_lock);
    curseg->cursor = curseg->active_start;
  } else {
  //ring buf next
    curseg->cursor = curseg->cursor + 1;
    if (curseg->cursor >= curseg->active_end) {
      curseg->cursor = curseg->active_start;
    } 
    else {
      curseg->cursor = curseg->cursor % max_size;
    }
  }
  ...
  if (segno == NULL_SEGNO){
    if (curseg->reclaimable_start != curseg->reclaimable_end) {
      ...
      if (curseg->reclaimable_start != curseg->reclaimable_end) {
        ...
        old_start = curseg->reclaimable_start++;
        if (curseg->reclaimable_start > 127) {
          curseg->reclaimable_start = 0;
        }
        curseg->active_zones[curseg->cursor] = curseg->reclaimable_zones[old_start];
      }
  ...
  // if inactive is not empty, change a zone into active
      ...
      if (curseg->inactive_start != curseg->inactive_end) {
        ...
        curseg->active_zones[curseg->cursor] = curseg->inactive_zones[old_start];
      }
      spin_unlock(&curseg->inactive_lock); 
      segno = curseg->active_zones[curseg->cursor];
    } else { 
...
#if STRIPE
static void allocate_segment_stripe(struct f2fs_sb_info *sbi,
						int type, bool force)
{
	struct curseg_info *curseg = CURSEG_I(sbi, type);
	//allocate segment
	new_curseg_striped(sbi, type);	
	//curseg->stripe_idx = (curseg->stripe_idx + 1) % curseg->stripe_idx;
...
#if STRIPE
static const struct segment_allocation stripe_salloc_ops = {
	.allocate_segment = allocate_segment_stripe,
};
#endif
...
#if 0
	if (fio->io_type == FS_CP_NODE_IO) {
		printk("(%s:%d) alloc_data_block of CP_NODE",
			__func__, __LINE__);
	}
#endif
	if (from_gc) {
...
#if SEP_SSA
static inline void init_sum_fio(struct f2fs_sb_info *sbi,
    struct f2fs_io_info *sum_fio,
    struct page *sum_page,
    struct f2fs_io_info *data_fio){

  struct f2fs_bio_info *io;


  /* copy basic info from data fio */
  sum_fio->type = data_fio->type;
  sum_fio->temp = data_fio->temp;
  sum_fio->op = data_fio->op;
  sum_fio->op_flags = data_fio->op_flags;
  sum_fio->io_type = FS_SEP_SSA_IO;
  sum_fio->encrypted_page = NULL;

  /* summary page info */
  sum_fio->new_blkaddr = sum_page->index;
  sum_fio->old_blkaddr = sum_page->index;
  sum_fio->page = sum_page;

  /* add io serialization list */
  INIT_LIST_HEAD(&sum_fio->list);
  sum_fio->in_list = true;
  io = sbi->write_io[sum_fio->type] + sum_fio->temp;
  spin_lock(&io->io_lock);
  list_add_tail(&sum_fio->list, &io->io_list);
  spin_unlock(&io->io_lock);
}
static inline void submit_sum_fio(struct f2fs_sb_info *sbi,
    struct f2fs_io_info *sum_fio) {
  clear_page_dirty_for_io(sum_fio->page);
  set_page_writeback(sum_fio->page);
  ClearPageError(sum_fio->page);
  f2fs_submit_page_write(sum_fio);
  dec_page_count(sbi, F2FS_DIRTY_META);
  unlock_page(sum_fio->page);
}
#endif
static void do_write_page(struct f2fs_summary *sum, struct f2fs_io_info *fio)
{
#if SEP_SSA
  unsigned int segno, sum_blkaddr;
  struct page *page;
...
#if SEP_SSA
// if a segment is exhausted, submit the sum page.
// if curseg->blkaddr == 0, the curseg is reset by this page write.
  if (CURSEG_I(fio->sbi, type)->next_blkoff == 0) {
    struct writeback_control wbc = {
      .for_reclaim = 0,
    };
    struct f2fs_io_info sum_fio = {
      .sbi = sbi,
      .ino = sbi->meta_ino_num,
      .io_wbc = &wbc,
    };
    ...
    submit_sum_fio(sbi, &sum_fio);
    f2fs_put_page(page, 0);
  }
  mutex_unlock(&SM_I(sbi)->ssa_mutex);
#endif
	if (fio->retry) {
...
#if SEP_SSA
	if (unlikely((io_type != FS_SEP_SSA_IO) && (page->index >= MAIN_BLKADDR(sbi))))
		fio.op_flags &= ~REQ_META;

  if (io_type == FS_SEP_SSA_IO) {
    fio.type = PAGE_SEP_SSA;
  }
#else
...
#if STRIPE
				(!IS_CURSEG(sbi, segno) || !is_inuse_seg(sbi, segno))) {
#else
				(!IS_CURSEG(sbi, segno))) {
#endif
					...
					block_t blkaddr, int type)
{
	...
	for (i = type; i < end; i++)
//#if META_FOR_ZNS
//		write_current_sum_log_page(sbi, i);
//#else
...
#if META_FOR_ZNS
/*
static struct page *get_current_sum_page(struct f2fs_sb_info *sbi,
		unsigned int segno)
{
	struct f2fs_sm_info *sm_i = SM_I(sbi);
	block_t blkaddr;
	...
	blkaddr = get_cur_meta_blkaddr(sbi, segno, sm_i->ssa_blkaddr, 
			sm_i->ssa_bitmap, 1);
	
	return f2fs_get_meta_page(sbi, blkaddr);
}
*/
static struct page *get_next_sum_page(struct f2fs_sb_info *sbi,
		unsigned int segno)
{
	struct f2fs_sm_info *sm_i = SM_I(sbi);
	struct page *page;
	...
	check_seg_range(sbi, segno);
	dst_off = get_next_meta_blkaddr(sbi, segno, sm_i->ssa_blkaddr, 
			sm_i->ssa_bitmap, 1);
					...
					unsigned int start)
{
	...
	pgoff_t dst_off;
#if META_FOR_ZNS
	dst_off = get_next_meta_blkaddr(sbi, SIT_BLOCK_OFFSET(start),
			sit_i->sit_base_addr, sit_i->sit_bitmap, 0);
#else
	pgoff_t src_off;
...
int flush_sum_blks(struct f2fs_sb_info *sbi, struct cp_control *cpc){
  ...
  if((err = __flush_sum_blks(sbi))){
	...
	if (fg_merge) {
		...
		if (err == -1) {
			err = 0;
		  printk("(%s : %d) fg_merge ssa error", __func__, __LINE__);
    }
		//add log zone reset
	}
	f2fs_submit_merged_write(sbi, META);
...
static void merge_ssa_set(struct f2fs_sb_info *sbi, struct ssa_set *set, int foreground){
	...
	struct f2fs_summary_block *raw_sum = NULL;
	struct f2fs_sm_info *sm_i = SM_I(sbi);

	
	page = get_next_sum_page(sbi, set->segno);
	raw_sum = page_address(page);

	memcpy(raw_sum->entries, set->entries, SUM_ENTRY_SIZE);
	memcpy(&raw_sum->footer, &set->footer, SUM_FOOTER_SIZE);
	
	sm_i->logged_sum_blks--;
	clean_ssa_set(sbi, set, foreground);
	...
	if (f2fs_sync_single_meta_page(page)) {
		unlock_page(page);
		printk("(%s : %d) error during sync ssa page(idx :%lu)",
...
int merge_ssa(struct f2fs_sb_info *sbi, int foreground){
	struct f2fs_sm_info *sm_i = SM_I(sbi);
	struct ssa_set *set, *next;
	struct ssa_set *setvec[SETVEC_SIZE];
	LIST_HEAD(sets);
...
//	int last_updated_zone = -1;
  ...
  if (foreground) {
    ...
    merge_tree_idx = SM_I(sbi)->cur_log_tree_idx ^ 0x1;
  }
...
*/
	...
	while ((found = radix_tree_gang_lookup(root, 
		...
		unsigned int idx;
		set_idx = setvec[found - 1]->segno + 1;
		for(idx=0;idx<found;idx++){
			list_add_tail(&setvec[idx]->set_list, &sets);	
		}
	}
	set = list_first_entry(&sets, struct ssa_set, set_list);
	...
	list_for_each_entry_safe(set, next, &sets, set_list){
		...
		if(cur_zone_offset != meta_boff_to_zoff(sbi, 
					set->segno)){
			if(wp < zone_cap) {
				wp = advance_meta_zone_wp(sbi, cur_zone_offset,
						wp, (zone_cap - wp), SSA);
		...
		merge_ssa_set(sbi, set, foreground);
		wp++;
...
static bool __mark_sit_log_dirty(struct f2fs_sb_info *sbi, unsigned int segno){
	...
	if(!__test_and_set_bit(segno, sit_i->sit_log_bitmap)){
		//sit_i->logged_sentries++;
		return false;
	}
	return true;
}
static void __insert_sit_log_set(struct f2fs_sb_info *sbi, unsigned int segno){
	unsigned int start_segno = START_SEGNO(segno);
	...
	if(!head){
		head = f2fs_kmem_cache_alloc(sit_entry_set_slab,
				GFP_NOFS, true, NULL);
		INIT_LIST_HEAD(&head->set_list);
		head->start_segno = start_segno;
		head->entry_cnt = 0;
		f2fs_radix_tree_insert(root, start_segno, head);
	}

	// mark dirty in sit log bitmap
...
//			printk("(%s : %d) segno: %u", __func__, __LINE__, segno);
			...
			if (!fg_merge) {
				...
				if(offset >= SIT_LOG_ENTRIES){
					...
					if (!has_curlog_space(sbi, 1, SIT_LOG))	{
						// prepare merge
						printk("(%s:%d) set merge flag", __func__, __LINE__);
						...
						printk("(%s:%d) switch sit log", __func__, __LINE__);
					}
...
static int merge_sit_set(struct f2fs_sb_info *sbi, struct sit_entry_set *set, int foreground){
	...
	unsigned int start_segno = set->start_segno;
	unsigned int end = min(start_segno + SIT_ENTRY_PER_BLOCK,
			(unsigned long)MAIN_SEGS(sbi));
	unsigned int segno = start_segno;
  ...
  if (foreground)
	...
	for_each_set_bit_from(segno, bitmap, end){
		...
		check_block_count(sbi, segno, &raw_sit->entries[sit_offset]);
  ...
  if (set->entry_cnt) {	
	...
	if (f2fs_sync_single_meta_page(page)) {
		unlock_page(page);
		printk("(%s : %d) error during sync sit page(idx : %lu)",
				__func__, __LINE__, page->index);
		return -1;
	}
	f2fs_put_page(page, 0);
...
int merge_sit(struct f2fs_sb_info *sbi, int foreground){
	struct f2fs_sm_info *sm_i = SM_I(sbi);
	struct sit_entry_set *set, *next;
	struct sit_entry_set *setvec[SETVEC_SIZE];
	LIST_HEAD(sets);

	unsigned int found;	
	unsigned int boff_in_zone = 0;
	unsigned int cur_zone_offset = 0;
	...
	if (foreground)
		...
		merge_tree_idx = sm_i->sit_ltree_idx ^ 0x1;
	...
	set = list_first_entry(&sets, struct sit_entry_set, set_list);
	cur_zone_offset = meta_boff_to_zoff(sbi, SIT_BLOCK_OFFSET(set->start_segno));
	reset_meta_zone_towrite(sbi, cur_zone_offset, SIT);

	list_for_each_entry_safe(set, next, &sets, set_list){

		if(cur_zone_offset != meta_boff_to_zoff(sbi, 
					SIT_BLOCK_OFFSET(set->start_segno))){
			if(wp < zone_cap) {
				wp = advance_meta_zone_wp(sbi, cur_zone_offset,
						wp, (zone_cap - wp), SIT_LOG);
				f2fs_bug_on(sbi, wp < 0);
			}
			cur_zone_offset = meta_boff_to_zoff(sbi, 
					SIT_BLOCK_OFFSET(set->start_segno));
			wp = 0;
			reset_meta_zone_towrite(sbi, cur_zone_offset, SIT);
		}
		boff_in_zone = meta_boff_in_zone(sbi, 
				SIT_BLOCK_OFFSET(set->start_segno));

		if(wp < boff_in_zone){
			wp = advance_meta_zone_wp(sbi, cur_zone_offset, wp,
					(boff_in_zone - wp), SIT_LOG);
			f2fs_bug_on(sbi, wp < 0);
		}
		if (merge_sit_set(sbi, set, foreground)) {
			return -1;
    }
		clean_sit_log_set(sbi, set, foreground);
		wp++;
	}
...
void f2fs_flush_sit_entries(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
  ...
  __flush_sit_log(sbi, cpc);

	//printk("(%s : %d) flush sit log done", __func__, __LINE__);
...
static int build_sit_info(struct f2fs_sb_info *sbi)
{
	...
	if (!sit_i->dirty_sentries_bitmap)
		return -ENOMEM;
#if META_FOR_ZNS
	sit_i->sit_log_bitmap = f2fs_kvzalloc(sbi, main_bitmap_size,
								GFP_KERNEL);
...
static int build_curseg(struct f2fs_sb_info *sbi)
{
	...
	ret = restore_curseg_summaries(sbi);
    ...
    array[i].active_start = 0;
    array[i].active_end = array[i].wanted_size;
    array[i].cursor = array[i].active_start;

    array[i].active_zones[array[i].active_start] = array[i].segno;

    array[i].inactive_start = 0;
    array[i].inactive_end = 0;
    array[i].reclaimable_start = 0;
    array[i].reclaimable_end = 0;
    
	  spin_lock_init(&array[i].active_lock);
	  spin_lock_init(&array[i].inactive_lock);
	  spin_lock_init(&array[i].reclaimable_lock);
...
#else
  unsigned int unusable_segs_in_sec;
#endif
...
// assume that all zones have the same unsuable segs
  unusable_segs_in_sec *= SM_I(sbi)->grid_cnt;
			...
			struct f2fs_sb_info *sbi, unsigned int segno)
{
  ...
  block_t sec_start_blkaddr, sec_cap_blkaddr;
	...
	if (!FDEV(dev_idx).zone_capacity_blocks)
		return sbi->blocks_per_seg;
#if SEP_SSA
  if (GET_SEG_OFFSET_IN_SEC(sbi, segno)
  ...
  if (IS_LAST_SEG(sbi, segno)) {
    ...
    return 0;
  if (seg_start + BLKS_PER_SUBSEG(sbi) > sec_cap_blkaddr)
    return ((sec_cap_blkaddr - seg_start) * SM_I(sbi)->grid_cnt);
#endif
	return sbi->blocks_per_seg;
}
...
int f2fs_build_segment_manager(struct f2fs_sb_info *sbi)
{
  ...
  int i;
	...
	ssa_bitmap_size = __bitmap_size(sbi, SSA_BITMAP);
	// code for recovery
	src_bitmap = __bitmap_ptr(sbi, SIT_BITMAP);
...
#endif
  for (i = 0; i<IG_NR; i++) {
    sm_info->data_alloc_IG[i] = CURSEG_NOT_ALLOC;
    sm_info->node_alloc_IG[i] = CURSEG_NOT_ALLOC;
    sm_info->free_sz_cnt[i] = MAIN_SECS(sbi) / IG_NR;
    if ((MAIN_SECS(sbi) % IG_NR) > i) {
...
#if META_FOR_ZNS
	ssa_set_slab = f2fs_kmem_cache_create("f2fs_ssa_set",
			sizeof(struct ssa_set));
	if (!ssa_set_slab)
		goto destroy_ssa_set;
#endif
	return 0;
#if META_FOR_ZNS
destroy_ssa_set:
	kmem_cache_destroy(ssa_set_slab);
#endif
